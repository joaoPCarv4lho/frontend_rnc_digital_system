...
const qualityColumns = [
    ...
    { key: "closing_date", label: "Data/hora Fechamento" }
];
...
export default function QualityDashboard(){
    ...
    const totalRncs = approvedRncs.items.length + refusedRncs.items.length;

    return(
      <div className="min-h-screen bg-gray-200">
        <Navbar title="Painel de Qualidade" icon={<CircleUserRound />}/>
        ...
        <div className="container mx-auto px-4 py-8">
          ...
          <motion.div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-10" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
            <KpiCard title="RNCs Aprovados" value={approvedRncs.items.length} icon={<CheckCircle className="text-green-600 w-6 h-6" />} color="bg-green-50" />
            <KpiCard title="RNCs Recusados" value={refusedRncs.items.length} icon={<XCircle className="text-red-600 w-6 h-6" />} color="bg-red-50" />
